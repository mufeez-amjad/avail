        ...
        if let Some((date, events)) = day {
            ...
            for event in events {
                ...
                if curr_time < start.time() {
                    ...
                    if avail_end.time() - avail_start.time() >= duration && avail_start.time() < max
                    {
            ...
            while curr.date() < end.date() || (curr.date() == end.date() && curr < end) {
                ...
                // Whole day
                let end = curr + (max - start.time());

                if start.time() >= max || end - start < duration {
                    break;
                }
...
mod tests {
    ...
    fn test_get_availability() {
        ...
        let avails = get_free_time(events, start, end, min, max, duration).unwrap();
    ...
    fn test_get_availability_rounding() {
        ...
        let start = create_local_datetime("10-05-2022 00:00");
        let end = create_local_datetime("10-06-2022 00:00");
        let min = NaiveTime::from_hms(9, 0, 0);
        let max = NaiveTime::from_hms(17, 0, 0);
        let duration = Duration::minutes(30);
